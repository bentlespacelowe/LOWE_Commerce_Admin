...
class Sboardlist extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
        ...
        if (res.data) {
          ...
          for (let i = 0; i < res.data.length; i++) {
            ...
            if (res.data[i].funnel === '/' || !res.data[i].funnel) {
              url = 'board/' + res.data[i].BoardId;
            } else {
              url = 'board/' + res.data[i].BoardId + '?' + funnel;
            }
            if (obj[url]) {
              obj[url].num++;
            } else {
              ...
              {Object.keys(this.state.showdata)
                ...
                .map((e, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <a href={'https://lowehair.kr/board/' + this.state.showdata[e].id}>{this.state.showdata[e].id}</a>
                    </td>
