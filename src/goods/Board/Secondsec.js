...
class Secondsec extends Component {
  ...
  componentDidMount = () => {
    ...
    if (id && user) {
      axios
        .post('http://15.165.44.114:5000/boardLikeChk', {
          user: user,
          id: id,
        })
        .then((res) => {
  ...
  async onclickLike(e) {
    ...
    if (id && user) {
      await axios
        .post('http://15.165.44.114:5000/boardLikeUpdate', {
          id: id,
          user: user,
          heart: like,
        })
        ...
        <div className='Board_second_section'>
          ...
          <div className='Board_second_price'>
            <div className='Board_price'>
              {this.props.data.board.eventType ? <span className='Board_price_percent'>{this.props.data.board.eventPrice}%</span> : <></>}
              <span className='Board_price_price'>{this.props.data.board.price.comma()}원</span>
            </div>
            <div>
              {this.state.like === false ? (
                <img
                  src={process.env.PUBLIC_URL + '/image/nav/goods_dislike.svg'}
                  className='Board_like'
                  alt='로위 상품 찜'
                  onClick={this.onclickLike}
                />
              ) : (
                <img
                  src={process.env.PUBLIC_URL + '/image/nav/goods_like.svg'}
                  className='Board_like'
                  alt='로위 상품 찜'
                  onClick={this.onclickLike}
                />
