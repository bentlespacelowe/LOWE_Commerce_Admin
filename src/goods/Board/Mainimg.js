    ...
    return (
      <>
        ...
        {this.props.sub.map((e) => (
          <div key={e.id}>
            <div className='Mainimg_sub' style={{ fontWeight: '700', marginTop: '100px' }}>
              {e.main}
            </div>
            {e.Urls.map((v) => (
              <>
                {v.url.slice(v.url.lastIndexOf('.'), v.url.lastIndexOf('.') + 4) === '.avi' ||
                v.url.slice(v.url.lastIndexOf('.'), v.url.lastIndexOf('.') + 4) === '.mp4' ? (
