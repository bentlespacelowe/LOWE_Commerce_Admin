  ...
  render() {
    return (
      <div className='Board_third_filter' id='scroll'>
        <a className={this.state.list === 1 ? 'push_button' : 'pull_button'} onClick={this.onclickList(1)} href='#goods'>
          상품설명
        </a>
        <a className={this.state.list === 2 ? 'push_button' : 'pull_button'} onClick={this.onclickList(2)} href='#review'>
          리뷰({this.props.data.board.Reviews.length})
        </a>
        <a className={this.state.list === 3 ? 'push_button' : 'pull_button'} onClick={this.onclickList(3)} href='#location'>
          매장 위치
        </a>
      </div>
    );
