...
class create extends Component {
  ...
  componentDidMount = () => {
    axios
      .post('http://54.180.117.244:5000/getDesignerList', {})
      .then((res) => {
  ...
  handleImgValue = () => {
    ...
    axios
      .post('http://54.180.117.244:5000/createBoard', {
        phone: '0',
  ...
  handleImgedit = async () => {
    ...
    await axios
      .post('http://54.180.117.244:5000/updateBoard', {
  ...
  render() {
    ...
    return (
      <div className='goods'>
        ...
        <div>
          <span>상품이름 </span>
          <input type='text' onChange={this.handleInputValue('name')} placeholder='상품이름' id='gname' />
        </div>
        <div>
          <span>상품설명 </span>
          <input type='text' onChange={this.handleInputValue('content')} placeholder='상품설명' id='gcontent' />
        </div>
        <div>
          <span>상품가격 </span>
          <input type='number' onChange={this.handleInputValue('price')} placeholder='가격(원)' id='gprice' />
        </div>
        <div>
        ...
        <div>
          ...
          <select name='eventType' id='geventType' onChange={this.handleInputValue('eventType')}>
            <option value={board.eventType ? board.eventType : ''}>
              {Number(board.eventType) === 0 ? '이벤트 없음' : Number(board.eventType) === 1 ? '이벤트 있음' : ''}
            </option>
        ...
        <select name='open' id='gopen' onChange={this.handleInputValue('open')}>
          <option value={board.open ? board.open : ''}>{board.open === '0' ? '비공개' : board.open === '1' ? '공개' : ''}</option>
        ...
        <div>
          ...
          <select name='gender' id='ggender' onChange={this.handleInputValue('gender')}>
            <option value={board.gender ? board.gender : ''}>{board.gender === 0 ? '남자' : board.gender === 1 ? '여자' : ''}</option>
        ...
        <div>
          ...
          <select name='length' id='glength' onChange={this.handleInputValue('length')}>
            <option value={board.length ? board.length : ''}>
              {board.length === 1 ? '숏' : board.length === 2 ? '미듐' : board.length === 3 ? '롱' : ''}
            </option>
        ...
        <div style={{ marginBottom: '30px', height: 'fit-content' }}>
          <div style={{ display: 'inline-block' }}>
            ...
            <div style={{ width: '375px', marginTop: '40px' }}>
              <input type='text' onChange={this.handleInputValue('MainimgMain')} placeholder='메인 설명' id='gMainimgMain' />
            </div>
            <div style={{ width: '375px', marginTop: '20px', marginBottom: '30px' }}>
              <input type='text' onChange={this.handleInputValue('MainimgSub')} placeholder='서브 설명' id='gMainimgSub' />
            </div>
          ...
          <div style={{ marginTop: '20px' }}>
            <div>
              {this.state.Subimgs.length
                ...
                : null}
              <input type='text' onChange={this.handleInputValue('SubimgSub')} placeholder='서브 설명' id='gSubimgSub' />
            </div>
        ...
        {this.props.location.state ? <button onClick={this.handleImgedit}>수정</button> : <button onClick={this.handleImgValue}>등록</button>}
      </div>
    );
  }
}
